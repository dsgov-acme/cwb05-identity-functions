...
class UserManagementClient {
    async createUser(request: UserCreateRequest, includeAppRoles: boolean): Promise<UserResponse> {
        logger.info('Calling create user with', { request });
        const includeApplicationRoles = `${includeAppRoles}`
        const paramters: Record<string, string> = { includeApplicationRoles };
    ...
    async getUserByExternalId(identityProvider: string, externalId: string, includeAppRoles: boolean): Promise<UserResponse | null> {
        const includeApplicationRoles = `${includeAppRoles}`
        const paramters: Record<string, string> = { identityProvider, externalId, includeApplicationRoles };
        const response = await fetch(`${config.userManagementBaseUrl}/api/v1/users?${new URLSearchParams(paramters)}`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',

...
resource "google_storage_bucket" "artifact_bucket" {
  name     = "identity-functions-${var.environment}-cwb05-dsgov-acme"
  location = var.region
...
resource "google_storage_bucket_object" "code_archive" {
  ...
  name   = "cwb05-identity-functions/${var.environment}/app.zip"
...
resource "google_cloudfunctions_function" "gcip_before_create" {
  ...
  source_archive_object        = "cwb05-identity-functions/${var.environment}/app.zip"
  trigger_http                 = true
  ...
  secret_environment_variables {
    ...
    secret     = "cwb05-self-signed-token-private-key"
    version    = "latest"
  }
  lifecycle {
